    ...
    def simulate(self, K, H):
        ...
        for k in range(K):
            ...
            for h in range(H):
                ...
                print("action: ",a_dict[a], "observation: (<",next_s[0],",", next_s[1], "> + enemy: ", enemy, "), reward: ", r)
                if enemy == 1:
                    self.switch_prob()
                    self.set_guard_pos()
            self.current_state = None
...
def main():
    # p = initial probability of enemy being at row 0
    # m = length of the corridor
    # K = number of episodes
    # H = horizon length

    pg = Prison_guard(1, 4) # ( p , m)
    # get dataset D of the form KxHx<action, row, column, enemy, reward> (i.e. KxHx5)
    D = pg.simulate(3, 9) # (K, H)
